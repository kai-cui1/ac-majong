...
| 2026-09-22 | **BL-038 副露原点失配修复（用户报障北家副露压牌背行）**：drawMelds 包裹节点内容=横排自原点向右（左缘原点）/竖排自原点向下（顶缘原点）；renderNorth 旧 `-totalW/2+meldW/2`、renderSouth 旧 `-160+meldW/2` 误按中心原点补偿致右移半块（北压牌背行、南明牌行右移逼近听牌钮）、renderSide 旧 `y=0` 致竖列下垂；修复三行：`-totalW/2` / `-160` / `meldH/2`；教训：**复用布局工厂须先确认其内容原点语义（左缘/顶缘/中心），定位补偿与原点语义错配=静默半块偏移**；?tableDemo=1 四家副露核对；Cocos typecheck 绿＋H5 构建＋对比图 cmp_melds_bl038 |

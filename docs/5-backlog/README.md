...
| BL-021 | 保底台数展示：台数徽章/浮层增「已锁定（保底）」口径（见花/杠含特番/门清当前态/已副露东风刻）；未听牌徽章显保底 N台（不再显 0），浮层保底区块常显 | 对局流程/体验 | 🟢 已完成（2026-09-19 用户确认 4 点） | 中 | PRD04 FR-对局-22；engine securedLines+TaiPreview.secured（复用 dedup 保口径）；TableScreen 徽章两态+浮层重构；game.html 原型同批；pipeline.test 4 例 |

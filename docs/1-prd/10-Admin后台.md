...
**二期范围（不在本 PRD 详展，仅登记）**：数据看板（DAU/对局量/台数分布）、运营配置（公告/规则参数）、用户处置（封禁/改资料）、**客户端诊断包受理（BL-023）**、**实时房间监控（BL-022）**。
...
| FR-Admin-08 | **导出单局回放包**（申诉取证，BL-024）：回放/对局详情页提供「导出回放包」，下载自包含、离线可确定性重演的 `ReplayBundle`（含 settings/seating/names/snapshot/actions）；operator+ | P1 | ❌ 待实现（复用 persistence `buildReplayBundle`）|
| FR-Admin-09 | **（二期）客户端诊断包受理**（BL-023）：粘贴玩家诊断包 JSON → 结构化展示（现场上下文/消息环/报错栈）→ 凭 room/round 跳回放仲裁 | P2 | ⬜ 二期 |
| FR-Admin-10 | **（二期）实时房间监控**（BL-022）：对卡顿的 playing 房查看实时态（在等谁/legalBySeat/停滞时长）；经 game-server 内网鉴权只读端点 | P2 | ⬜ 二期（需受控跨系统调用）|
